...
<head>
    ...
    <title>Global Citizenship Award - LILAC</title>
...
<body class="bg-background-light dark:bg-background-dark font-display text-text-light dark:text-text-dark">
    ...
    <main class="min-h-screen overflow-y-auto">
            <header class="sticky top-0 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-sm z-30 px-6 lg:px-8 py-4 border-b border-border-light dark:border-border-dark flex justify-between items-center h-20">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-text-light dark:text-text-dark">Global Citizenship Award</h1>
                        <p class="text-sm text-text-muted-light dark:text-text-muted-dark">Central Hub for Global Citizenship Award Documents</p>
                    </div>
                </div>
            </header>
            ...
            <div class="p-6">
                <div class="max-w-7xl mx-auto">
                    <!-- Award Overview -->
                    <div class="mb-8 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl p-6 border border-blue-200 dark:border-blue-700">
                        <div class="flex items-center gap-4 mb-4">
                            <div class="w-12 h-12 bg-blue-600 rounded-lg flex items-center justify-center">
                                <span class="material-symbols-outlined text-white text-2xl">public</span>
                            </div>
                            <div>
                                <h2 class="text-2xl font-bold text-blue-900 dark:text-blue-100">Global Citizenship Award</h2>
                                <p class="text-blue-700 dark:text-blue-300">Recognition for institutions demonstrating exceptional global engagement and citizenship</p>
                            </div>
                        </div>
                        ...
                        <div class="p-6">
                            <div id="documentsList" class="space-y-4">
                                ...
                                <div id="emptyState" class="hidden text-center py-12">
                                    ...
                                    <p class="text-text-muted-light dark:text-text-muted-dark">No documents match the Global Citizenship Award criteria.</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        // Award configuration
        const AWARD_NAME = 'Global Citizenship Award';
        const AWARD_KEYWORDS = [
            'global citizenship', 'global engagement', 'intercultural understanding', 
            'community engagement', 'inclusive programs', 'ethical governance', 
            'sustainable impact', 'global leadership', 'international partnership',
            'cross-cultural', 'diversity', 'global awareness'
        ];
        ...
        // Load documents that match this award
        async function loadAwardDocuments() {
            try {
                // Load all awards from the API
                ...
                // Filter documents that match this award
                const matchingDocuments = [];
                
                for (const award of allAwards) {
                    try {
                        // Get analysis data for this award
                        ...
                        if (analysisResponse.ok) {
                            ...
                            // Check if analysis results mention this award
                            if (analysis.analysis_results) {
                                try {
                                    ...
                                    const hasMatch = analysisResults.some(result => 
                                        ...
                                        // Also check for keyword matches in the document
                    ...
                    <div class="border border-border-light dark:border-border-dark rounded-lg p-4 hover:shadow-md transition-shadow">
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <button onclick="viewDocumentDetails('${doc.id}')" class="px-3 py-1 bg-primary text-white text-sm rounded-lg hover:bg-primary/90 transition-colors">
                                    View Details
                                </button>
                            </div>
                        </div>
                        ${matchingResult && matchingResult.recommendation ? `
                            <div class="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                <p class="text-sm text-blue-700 dark:text-blue-300">
                                    <strong>Recommendation:</strong> ${matchingResult.recommendation}
                                </p>
                            </div>
                        ` : ''}
                    </div>
        ...
        function updateStats() {
            ...
            const eligibleCount = allDocuments.filter(doc => {
                const status = doc.matchingResult ? doc.matchingResult.status : 'Unknown';
                return status.toLowerCase() === 'eligible';
            }).length;
            
            document.getElementById('eligibleCount').textContent = eligibleCount;
            
            const progressPercentage = allDocuments.length > 0 ? 
                Math.round((eligibleCount / allDocuments.length) * 100) : 0;
        ...
        async function viewDocumentDetails(awardId) {
            try {
                ...
                populateDocumentDetailsModal(data);
        ...
        function populateDocumentDetailsModal(data) {
                                ...
                                <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                                    ...
                                    <div class="mb-2">
                                        <div class="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400 mb-1">
                                            ...
                                            <span>${typeof result.score === 'number' && result.score <= 1 ? (result.score * 100).toFixed(1) : Number(result.score || 0).toFixed(1)}%</span>
                                        </div>
                                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                            <div class="bg-primary h-2 rounded-full" style="width: ${Math.min((typeof result.score === 'number' && result.score <= 1 ? (result.score * 100) : Number(result.score || 0)), 100)}%"></div>
                                        </div>
                                    </div>
                                    <div class="mb-2">
                                        ...
                                        <div class="mt-3">
                                            <details class="group">
                                                ...
                                                <div class="mt-2 space-y-1">
                                                    ...
                                                    `).join('')}
                                                    <div class="text-xs text-gray-500 dark:text-gray-400">${Number(result.checklist.percentage_met || 0).toFixed(2)}% met</div>
                                                </div>
                                            </details>
                                        </div>
                                    ` : ''}
        ...
        // Redirect script for port handling
        (function() {
            try {

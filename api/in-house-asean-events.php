...
try {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS inhouse_asean_events (
    ...
    if ($method === 'GET') {
        ...
        if ($action === 'restore') {
            ...
            $st = $pdo->prepare("UPDATE inhouse_asean_events SET deleted_at=NULL WHERE id=? LIMIT 1"); $st->execute([$id]);
            if ($st->rowCount()<=0) throw new Exception('Record not found');
            echo json_encode(['success'=>true,'message'=>'Record restored']); exit();
        }
        $st = $pdo->query("SELECT id, event_name, partners, countries, participants, year, created_at, updated_at FROM inhouse_asean_events WHERE deleted_at IS NULL ORDER BY created_at DESC");
    ...
    if ($method === 'POST') {
        ...
        if ($eventName === '') throw new Exception('Event name is required');
        ...
        if ($action === 'update') {
            $id = (int)($_GET['id'] ?? 0); if ($id<=0) throw new Exception('ID is required');
            $st = $pdo->prepare("UPDATE inhouse_asean_events SET event_name=?, partners=?, countries=?, participants=?, year=?, updated_at=NOW() WHERE id=? LIMIT 1");
            $st->execute([$eventName, $partners===''?null:$partners, $countries===''?null:$countries, $participants, $year, $id]);
            if ($st->rowCount()===0){$chk=$pdo->prepare("SELECT id FROM inhouse_asean_events WHERE id=? LIMIT 1");$chk->execute([$id]);if(!$chk->fetchColumn()) throw new Exception('Record not found');}
            echo json_encode(['success'=>true,'message'=>'Record updated']); exit();
        }
        $st = $pdo->prepare("INSERT INTO inhouse_asean_events (user_id, event_name, partners, countries, participants, year) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    if ($method === 'DELETE') {
        ...
        if ($permanent) { $st=$pdo->prepare("DELETE FROM inhouse_asean_events WHERE id=? LIMIT 1"); $st->execute([$id]); echo json_encode(['success'=>true,'message'=>'Deleted']); exit(); }
        $st = $pdo->prepare("UPDATE inhouse_asean_events SET deleted_at=NOW() WHERE id=? AND deleted_at IS NULL LIMIT 1"); $st->execute([$id]);
        if ($st->rowCount()<=0) throw new Exception('Record not found');
